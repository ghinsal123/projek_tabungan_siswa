...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $jumlah_penarikan = isset($_POST['jumlah_penarikan']) ? trim($_POST['jumlah_penarikan']) : '';
    ...
    if (empty($jumlah_penarikan) || !is_numeric($jumlah_penarikan) || $jumlah_penarikan <= 0) {
        echo "<script>alert('Masukkan jumlah penarikan yang valid!'); window.history.back();</script>";
        exit;
    }
    ...
    if ($db->query($query)) {
        ...
        echo "<script>alert('Gagal melakukan penarikan: " . $db->error . "'); window.history.back();</script>";
    }
}
?>
...
<div class="container">
    ...
    <form method="POST">
        ...
        <input type="number" name="jumlah_penarikan" id="jumlah_penarikan" min="1000" required>
